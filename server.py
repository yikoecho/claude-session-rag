...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/hybrid":
            if not query:
                ...
                return
            top_k = int(params.get("top_k", [5])[0])
            retro_q = params.get("retro_q", [""])[0]
            import time as _time
            print(f"[hybrid_server_recv] {_time.strftime('%H:%M:%S')} q={repr(query[:120])} retro_q={repr(retro_q[:60])}", flush=True)
            try:
                results = hybrid_search(query, top_k, retro_query=retro_q)
            except Exception as e:
                results = []
                print(f"[hybrid] 出错: {e}")
    ...
    def do_POST(self):
        ...
        recall_result = recall_agent(prompt, candidates)

        body = json.dumps({
            "verdict": recall_result["verdict"],
            "results": recall_result["selected"],
